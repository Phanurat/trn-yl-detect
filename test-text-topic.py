...
client = genai.Client(api_key='AIzaSyDICbQ-kxORGn1QD_UAZUt04zy4LLXC6K4')
...
for i, (box, conf) in enumerate(zip(result.boxes.xyxy, result.boxes.conf)):
    if conf >= confidence_threshold:  # ตรวจสอบความแม่นยำ
        ...
        if cropped_img.size > 0:
            ...
            promt = "วิเคราะห์อารมณ์และเนื้อหาของโพสต์นี้ วิเคราะห์ความคิดเห็นที่สำคัญจากโพสต์นี้ ระบุผลกระทบที่โพสต์นี้อาจมีต่อผู้ชม แล้วแสดงความคิดเห็นหน่อย สั้นๆ"
            # แสดงผลลัพธ์
            print("ข้อความที่ตรวจพบ:")
            print(text)
            promt_gen = (promt,text)
